...
(async function () {
  ...
  const parser = await import(
    "https://unpkg.com/simfile-parser@0.8.0/dist/browser/index.js"
  );
  ...
  document.body.addEventListener("drop", async (evt) => {
    ...
    try {
      ...
      document.querySelector("input[name='simfileartist']").value =
        simfile.artist;
      // max min bpm
      ...
      for (const chart of simfile.availableTypes) {
        try {
          ...
          document.querySelector(`select[name='${inputName}']`).value =
            chart.feet;
        } catch (e) {
